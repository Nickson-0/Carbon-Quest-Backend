...
const productionHosts = [
  'https://fifa.sauravmh.com',
  'https://sauravmh.vercel.app',
  'playfifa.vercel.app',
];
const devHosts = ['http://localhost:8080'];

exports.config = () => {
  IS_DEVELOPMENT: getDefault(process.env.NODE_ENV, 'development') !==
    'production';

  ALGOLIA_ID: getDefault(process.env.ALGOLIA_ID, 'latency');
  ALGOLIA_SEARCH_API_KEY: getDefault(
  ...
  ALLOWLIST_HOSTS: getDefault(process.env.NODE_ENV, 'development') ===
  'production'
    ? productionHosts
    : devHosts;

  ROOM_ID_RX: /^([A-Z\d]){6}$/;
  ATTRIBUTES_TO_RETRIEVE: getDefault(process.env.NODE_ENV, 'development') ===
  'production'
    ? [
        'name',
        'positions',
  ...
  METADATA_NETWORK_INTERFACE_URL: getDefault(
    process.env.METADATA_NETWORK_INTERFACE_URL,
    'REDACTED'
  );
  getExternalIp: async () => {
    const options = {
